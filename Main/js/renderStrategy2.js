...
$(function () {
    ...
    function start() {
        ...
        var treeJSON = d3.json("./js/new.json", function (error, treeData) {
            treeData = finalNode;
            var stratName = $("#stratname").val($("#selectionName").attr('value'));
            ...
            function addNode(d, id) {
                ...
                if (d.id === id) {
                    if (!d.children) {
                        d.children = [{"name": "Node", "id": newid, "targetpct": 0}];
                    }
                    else {
                        d.children.push({"name": "Node", "id": newid, "targetpct": 0});
                    }
                    update(root);
                    centerNode(d);
                    return;
                        ...
                        .on('click', function (d) {
                            ...
                            d3.select(this.parentNode.parentNode).append("image")
                                    ...
                                    .attr({
                                        ...
                                        'x': d3.select(this.parentNode)[0][0].__data__.x0 - 85,
            ...
            $("#submit").on("click", function () {
                ...
                if (checkFlag) {
                    ...
                    if (stratName === "" || stratName === null) {
                        alert("empty strategy name");
                    }
                    else {
                        $.ajax({
                            url: 'saveStrategy.php',
                            type: 'post',
                            data: {'tree': saveArray, 'Stratname': $("#selectionName").val(), 'phase': 'edit', 'Stratid': $("#selection").val()},
                            success: function (data, status) {
